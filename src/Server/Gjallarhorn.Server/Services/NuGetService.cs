...
namespace Gjallarhorn.Server.Services
{
    public class NuGetService : INuGetService
    {
        ...
        public async Task<string> GetLatestVersionAsync(string packageName, string packageUrl, bool comparePrerelease)
        {
            ...
            var results =  await searchResource.SearchAsync(packageName, new SearchFilter(comparePrerelease), 0, 1, logger, CancellationToken.None);
            var package = results.FirstOrDefault();
            if(package != null)
            {
                var versions = await package.GetVersionsAsync();
                return versions.First().Version.ToString();
